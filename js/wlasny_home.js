...
jQuery(document).ready(function($){
      var wrap = jQuery(".navbar-fly");
    var position = jQuery(".navbar-static-top").position();
     jQuery(window).scroll(function () {
         var scroll = jQuery(window).scrollTop();
        if ( scroll >= position.top+70){
            wrap.addClass("my_navbar-fixed-top navbar navbar-inverse navbar-fixed-top");
        }else {
            wrap.removeClass("my_navbar-fixed-top navbar navbar-inverse navbar-fixed-top");
        }
       
//        var scrolledY = $(window).scrollTop();
//        $('.gieroj_srodek_stable').css('background-position', 'left ' + ((scrolledY)) + 'px');
    });
